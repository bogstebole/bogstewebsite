...
import { QUESTIONS, RUNNING_PLAN, scriptedApi, threadReply, scriptedTranscript } from "@/lib/inline-chat/scriptedApi";
/* The playground pulls this in from its own entry point; a Next route has no
   such place, so the page that uses the kit imports the kit's stylesheet.
   Without it every `--ick-` token is unset and the page renders unstyled. */
...
export default function InlineChatExperiencePage() {
   ...
   */
  const [chosen, setChosen] = useState<"light" | "dark" | null>("light");
