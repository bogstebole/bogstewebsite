...
const DEFAULTS = {
  ...
  maxWaves:      4,     // n       — concurrent wave cap
}
 ...
 * Fires on click. Displaces all text characters and images inside.
 *
 * @example
 * const ref = useRippleWave({ textStrength: 60 })
 * <div ref={ref}>Any content</div>
 */
export function useRippleWave(options = {}) {
  ...
  useEffect(() => {
    ...
    const state = {
      spans:   [],   // [{ el, cx, cy }]
      images:  [],   // [{ canvas, ctx, base, w, h, wrapper }]
      waves:   [],   // [{ x, y, t }]
      raf:     null,
      alive:   true,
    }
      ...
      const walker = document.createTreeWalker(
        ...
        NodeFilter.SHOW_TEXT,
        {
          acceptNode(node) {
            ...
            if (p.dataset.rw) return NodeFilter.FILTER_REJECT  // already split
    ...
    function measureSpans() {
      ...
      state.spans = []
      container.querySelectorAll('[data-rw]').forEach(el => {
    ...
    function setupImages() {
      container.querySelectorAll('img').forEach(img => {
        ...
        // draw img into offscreen to get base pixel data
    ...
    function waveOffset(cx, cy, wx, wy, elapsed) {
    ...
    function onClick(e) {
      // re-measure on every click — handles dynamic content / layout shifts
      measureSpans()
    ...
    // wait two frames so the DOM is fully painted before measuring
    ...
    return () => {
      ...
      // restore original text nodes from split spans
      container.querySelectorAll('[data-rw]').forEach(span => {
      ...
      // remove canvas overlays and unwrap images
      state.images.forEach(({ canvas, w, h }) => {
