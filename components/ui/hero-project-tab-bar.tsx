...
interface HeroProjectTabBarProps {
  active: HeroProjectKey;
  onChange: (key: HeroProjectKey) => void;
}

export function HeroProjectTabBar({ active, onChange }: HeroProjectTabBarProps) {
  const [hovered, setHovered] = useState<HeroProjectKey | null>(null);

  const [prevActive, setPrevActive] = useState<HeroProjectKey>(active);
  const [transformOrigin, setTransformOrigin] = useState("center");

  if (active !== prevActive) {
    const prevIndex = HERO_PROJECT_ORDER.indexOf(prevActive);
    const currentIndex = HERO_PROJECT_ORDER.indexOf(active);
      ...
      {TABS.map((tab) => {
        ...
        return (
          ...
          >
            {isActive && (
              <motion.div
                layoutId="active-hero-tab-indicator"
