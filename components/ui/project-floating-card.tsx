...
export function ProjectFloatingCard({
  ...
  /* ── Show content once expand animation completes ── */
  ...
  const handleExpandComplete = useCallback(() => {
    ...
    setPhase(prev => (prev === "expanding" ? "open" : prev));
  ...
  const cardContent = (
    <>
      ...
      {/* Project-specific inner content */}
  ...
  return (
    <>
      ...
      {/* FLIP path — position:absolute inside fixed scroll wrapper */}
      <div
        onClick={() => void initiateClose()}
        style={{
          position: "fixed",
